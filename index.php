...
.dropdown-menu {
    ...
    position: absolute;
    top: 100%; /* Positionner sous le lien "PROJETS" */
    left: 0;
...
/* Afficher le menu déroulant lorsque la souris survole "PROJETS" */
...
// Requête SQL pour récupérer les projets et leurs catégories
$sql = "
    SELECT p.id AS projet_id, p.titre, p.annee, p.image_url, c.nom AS categorie
    FROM projets p
...
<div class="projets">
    <?php foreach ($projets as $projet): ?>
        <div class="projet" data-categories="<?= htmlspecialchars($projet['categorie'] ?? ''); ?>">
            <img src="<?= htmlspecialchars($projet['image_url'] ?? 'default-image.jpg'); ?>" 
                 alt="<?= htmlspecialchars($projet['titre'] ?? 'Image sans titre'); ?>">
            <div class="categorie">
                <p><?= htmlspecialchars($projet['categorie'] ?? 'Pas de catégorie'); ?></p>
                <div class="infos">
                    <div class="titre">
                        <h1><?= htmlspecialchars($projet['titre'] ?? 'Titre manquant'); ?></h1>
                    </div>
                    <div class="date">
                        <h1><?= htmlspecialchars($projet['annee'] ?? 'Année inconnue'); ?></h1>
                    </div>
                </div>
            </div>
        </div>
...
.projets {
    display: flex;
    flex-wrap: wrap; /* Permet d'aller à la ligne si les éléments débordent */
    justify-content: center; /* Centre horizontalement les images */
    gap: 20px; /* Espace entre les images */
    margin: 20px auto; /* Ajoute de l'espace autour du conteneur */
}

/* Style individuel pour chaque projet */
.projets .projet {
    flex: 0 1 370px; /* Taille fixe pour chaque projet (370px de large) */
    display: flex;
    flex-direction: column; /* Permet d'empiler l'image et les infos */
    align-items: center; /* Centre le contenu horizontalement */
    text-align: center; /* Centre les textes (titre, année) */
    box-shadow: 0 4px 8px rgba(0, 0, 0, 0.1); /* Ajoute une ombre légère */
    padding: 10px; /* Ajoute un peu de padding autour des éléments */
    background-color: #f9f9f9; /* Couleur de fond légère pour un effet propre */
    border-radius: 5px; /* Arrondi optionnel des bords */
}

/* Assurez-vous que toutes les images des projets aient la même taille */
.projets .projet img {
...
/* Informations supplémentaires sous les images */
.projets .projet .categorie {
    margin-top: 10px; /* Ajoute un espace entre l'image et les textes */
}

.projets .projet .categorie p {
    font-size: 14px;
    color: #555; /* Couleur de texte légèrement grisée */
    margin: 5px 0;
}

.projets .projet .infos h1 {
    font-size: 16px;
    color: #333; /* Couleur de texte plus foncée pour les titres */
    margin: 5px 0;
}
</style>
